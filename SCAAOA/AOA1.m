...
% 仅加入自适应MOA策略改进的算术优化算法AOA1
function [Best_FF,Best_P,Conv_curve]=AOA1(N,M_Iter,LB,UB,Dim,F_obj)
...
%     MOA=MOA_Min+C_Iter*((MOA_Max-MOA_Min)/M_Iter);   %更新数优化器加速函数
    ...
    for i=1:size(X,1)
        ...
        MOA = 1-((Worst_FF-Ffun(i))/(Worst_FF-Best_FF))^k;%自适应数学优化器加速函数
